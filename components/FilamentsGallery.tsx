...
export default function FilamentsGallery({ entries }: Props) {
...
function GalleryTile({
  ...
  return (
    <button
      ...
      className="tile group relative mb-3 block w-full cursor-zoom-in overflow-hidden rounded-[2px] bg-cream/5 focus:outline-none focus-visible:ring-1 focus-visible:ring-gold/60"
      style={{ aspectRatio: `${entry.w} / ${entry.h}` }}
    >
      <Image
        src={`/filaments/thumb/${entry.id}.avif`}
        alt={entry.alt}
        fill
...
function Lightbox({
  ...
  return (
    ...
    >
      ...
      >
        <Image
          src={`/filaments/full/${entry.id}.avif`}
          alt={entry.alt}
          width={entry.w}
          height={entry.h}
