// Preprocess Filaments gallery source images into small AVIFs + a manifest.
// Run from the project root: `node scripts/process-filaments.mjs`
//
// Inputs  : content/filaments/*.{png,jpg,jpeg,webp,tif,tiff}
// Outputs : public/filaments/thumb/<id>.avif  (~800px long edge)
//           public/filaments/full/<id>.avif   (~2400px long edge)
//           content/filaments-manifest.json   (id, w, h, blurDataURL per image)
...
const SRC_DIR = path.join(ROOT, "content", "filaments");
const OUT_DIR = path.join(ROOT, "public", "filaments");
const OUT_THUMB = path.join(OUT_DIR, "thumb");
const OUT_FULL = path.join(OUT_DIR, "full");
const MANIFEST = path.join(ROOT, "content", "filaments-manifest.json");
...
async function processOne(idx) {
  ...
  const id = `filament-${String(idx + 1).padStart(3, "0")}`;
  ...
  manifest[idx] = {
    ...
    h: meta.height ?? 1,
    blurDataURL,
    alt: `Filaments — flow-field bead chain variation ${idx + 1}`,
  };
